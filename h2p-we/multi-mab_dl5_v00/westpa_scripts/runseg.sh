...
ln -sv $WEST_SIM_ROOT/common_files/1ii6_dl5_solv.prmtop .
...
$PMEMD -O -i md.in   -p 1ii6_dl5_solv.prmtop  -c parent.ncrst \
          -r seg.ncrst -x seg.nc      -o seg.log    -inf seg.nfo

COMMAND="         parm 1ii6_dl5_solv.prmtop\n"
...
COMMAND="$COMMAND rmsd :18-357&!@H= reference \n"
# pcoord calc
# pcoord 1: ADP+Mg RMSD
COMMAND="${COMMAND} rmsd adp-mg-rmsd :362-363&!@H= reference nofit out pcoord.dat \n"
...
COMMAND="$COMMAND energy cpx-ene :1-363 out pcoord-ene.dat \n"
COMMAND="$COMMAND energy rec-ene :1-361 out pcoord-ene.dat \n"
COMMAND="$COMMAND energy adp-ene :362,363 out pcoord-ene.dat \n"
#COMMAND="$COMMAND energy mon-ene :371 out pcoord-ene.dat \n"

# pcoord 3: min contact distance of ADP and Eg5
COMMAND="${COMMAND} nativecontacts mindist :1-361 :363 out pcoord.dat \n"

# pcoord 4: min contact distance of PO4 and eg5
COMMAND="${COMMAND} nativecontacts mindist :1-361 :363@O1B,PB,O2B,O3B,O3A,PA,O1A,O2A,O5*,C5* out pcoord.dat \n"

COMMAND="$COMMAND rmsd base-rmsd :363@C4*,O4*,C1*,N9,C8,N7,C5,C6,N6,N1,C2,N3,C4,C3*,O3*,C2*,O2* reference nofit out adp_rmsd.dat \n"
COMMAND="$COMMAND rmsd phos-rmsd :363@O1B,PB,O2B,O3B,O3A,PA,O1A,O2A,O5*,C5* reference nofit out adp_rmsd.dat \n"
# begin auxdata_rmsd.dat
COMMAND="$COMMAND rmsd prot-rmsd :1-361&!@H= reference nofit out auxdata_rmsd.dat \n"
COMMAND="$COMMAND rmsd rec-rmsd :24-30,72-78,81,103-118,125,225-226,258-263,267,326,328-331&!@H= reference nofit out auxdata_rmsd.dat \n"
COMMAND="$COMMAND rmsd adp-rmsd :363&!@H= reference nofit out auxdata_rmsd.dat \n"
COMMAND="$COMMAND rmsd adpcpx-rmsd :1-363&!@H= reference nofit out auxdata_rmsd.dat \n"
#COMMAND="$COMMAND rmsd loop5-rmsd :116-133&!@H= reference nofit out auxdata_rmsd.dat \n"
COMMAND="$COMMAND rmsd protcut-rmsd :18-357&!@H= reference nofit out auxdata_rmsd.dat \n"
...
COMMAND="$COMMAND surf rec-sasa :24-30,72-78,81,103-118,125,225-226,258-263,267,326,328-331 out auxdata_sasa.dat \n"
COMMAND="$COMMAND surf adp-sasa :363 out auxdata_sasa.dat \n"
#COMMAND="$COMMAND surf mon-sasa :371 out auxdata_sasa.dat \n"
COMMAND="$COMMAND surf prot-sasa :1-361 out auxdata_sasa.dat \n"
COMMAND="$COMMAND go\n"
...
cat adp_rmsd.dat | tail -n +2 | awk {'print $2'} > $WEST_ADP_BASE_RMSD_RETURN
cat adp_rmsd.dat | tail -n +2 | awk {'print $3'} > $WEST_ADP_PHOS_RMSD_RETURN
...
cat auxdata_rmsd.dat | tail -n +2 | awk {'print $6'} > $WEST_PROTCUT_RMSD_RETURN

cat pcoord-ene.dat | tail -n +2 | awk {'print $10'} > $WEST_RECEPT_ENE_RETURN
cat pcoord-ene.dat | tail -n +2 | awk {'print $19'} > $WEST_ADP_ENE_RETURN
cat pcoord-ene.dat | tail -n +2 | awk {'print $28'} > $WEST_ADPCOMPLEX_ENE_RETURN
#cat pcoord-ene.dat | tail -n +2 | awk {'print $37'} > $WEST_MON_ENE_RETURN
...
cp 1ii6_dl5_solv.prmtop $WEST_TRAJECTORY_RETURN
cp seg.nc $WEST_TRAJECTORY_RETURN

cp 1ii6_dl5_solv.prmtop $WEST_RESTART_RETURN
cp seg.ncrst $WEST_RESTART_RETURN/parent.ncrst

cp seg.log $WEST_LOG_RETURN

# strip water
CMD="     parm $WEST_SIM_ROOT/common_files/1ii6_dl5_solv.prmtop \n"
...
# Clean up
rm -f md.in parent.ncrst seg.nfo seg.pdb 1ii6_dl5_solv.prmtop 05_eq3.ncrst *.dat *.txt
