...
paste <(cat pcoord.dat | tail -n +2 | awk '{print $2}') <(cat int_ene_proc.txt) <(cat pcoord.dat | tail -n +2 | awk '{print $8}') > $WEST_PCOORD_RETURN

# mdtraj sasa calc
python $WEST_SIM_ROOT/common_files/analyze_sasa.py $WEST_CURRENT_SEG_DATA_REF
# calc interaction energy from aux, not using <8A updating definition
bash $WEST_SIM_ROOT/common_files/get_energies.sh auxdata_ene.dat aux_int_ene.txt
python $WEST_SIM_ROOT/common_files/get_energies.py aux_int_ene.txt aux_int_ene_proc.txt

cat pcoord.dat | tail -n +2 | awk {'print $3'} > $WEST_ADP_PROT_COM_DIST_RETURN
cat pcoord.dat | tail -n +2 | awk {'print $4'} > $WEST_ADP_BASE_RMSD_RETURN
cat pcoord.dat | tail -n +2 | awk {'print $5'} > $WEST_ADP_PHOS_RMSD_RETURN
...
cat aux_int_ene_proc.txt > $WEST_STATIC_INT_ENE_RETURN
