...
class Kinetics:
    ...
    def extract_rate(self):
        ...
        # # min non-zero value
        # min_val = np.amin(state_pop_a[np.nonzero(state_pop_a)])
        # # replace all zeros with smallest value measured
        # state_pop_a[state_pop_a == 0] = min_val
    ...
    def plot_rate(self, title=None, moltime=True, color=None, linestyle="-"):
        """
        ...
        if moltime:
            ...
            #self.tau *= 10**-9
            # multiply by tau
            #iterations = np.multiply(iterations, self.tau)
            # convert to ns
            #iterations = np.divide(iterations, 10**9)

            iterations = np.multiply(iterations, 50)
        ...
        if self.units == "mfpts":
            ...
            self.ax.plot(iterations, rate_ab, label=self.label, linestyle=linestyle, color=color)
            self.ax.fill_between(iterations, rate_ab - ci_lb_ab, rate_ab + ci_ub_ab, alpha=0.5, color=color)
            self.ax.set_ylabel("Rate Constant ($s^{-1}$)")
            print(f"{self.scheme}: {rate_ab[-1]} +{ci_ub_ab[-1]}-{ci_lb_ab[-1]}")
...
#k = Kinetics(scheme=f"ANALYSIS2", state=1, ax=ax, tau=5**-10)
#k = Kinetics(scheme=f"WIPA/TEST", state=1, ax=ax, tau=5**-10)
#k.plot_rate()

# k = Kinetics(scheme=f"eg5_wt/WIPA/TO_STUCK", state=1, ax=ax, tau=5**-10, label="WT k$_1$")
# k.plot_rate(color="lightsalmon")
# k = Kinetics(scheme=f"eg5_mon/WIPA/TO_STUCK", state=1, ax=ax, tau=5**-10, label="MB k$_1$")
# k.plot_rate(color="lightsteelblue")

k = Kinetics(scheme=f"eg5_wt/WIPA/FULLY_UNBOUND2", state=1, ax=ax, tau=5**-10, label="WT k$_{overall}$")
k.plot_rate(color='firebrick')
k = Kinetics(scheme=f"eg5_wt/WIPA/STUCK_TO_UNBOUND", state=1, ax=ax, tau=5**-10, label="WT k$_2$")
k.plot_rate(color='tomato')
k = Kinetics(scheme=f"eg5_mon/WIPA/FULLY_UNBOUND2", state=1, ax=ax, tau=5**-10, label="MB k$_{overall}$")
k.plot_rate(color='royalblue')
k = Kinetics(scheme=f"eg5_mon/WIPA/STUCK_TO_UNBOUND", state=1, ax=ax, tau=5**-10, label="WT k$_2$")
k.plot_rate(color='cornflowerblue')

# ax.axhline(0.05, color="grey", linestyle="--")
# ax.axhline(0.007, color="grey", linestyle="--")
#ax.axhline(0.27, color="grey", label="Rate from experiment")
plt.tight_layout()
#plt.ylim(1e-54, 300)

plt.legend()
plt.show()
fig.savefig("eg5_k2s.png", dpi=600, transparent=True)
